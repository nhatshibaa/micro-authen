...
//    public static final String LOGIN_FORM_CLIENT_SECRET_KEY = "client_secret";
...
//        params.put(LOGIN_FORM_CLIENT_SECRET_KEY, KeycloakConstant.KEYCLOAK_CLIENT_SECRET);
    ...
    @Override
    public boolean addRealmRoleById(String id, RoleRepresentation[] roleRepresentations) throws IOException {
        prepareAdminToken();
        RetrofietUserService service
                = RetrofietServiceGenerator.createService(RetrofietUserService.class, adminToken);
        Response<RoleRepresentation> response
                = service.saveRealmRole(id, roleRepresentations).execute();
        if (!response.isSuccessful()) {
            if (response.code() == HttpStatus.UNAUTHORIZED.value()
                    || response.code() == HttpStatus.FORBIDDEN.value()) {
                adminToken = null;
    ...
    @Override
    public boolean addClientRoleById(String id, String client, ClientRoleRepresentation[] clientRoleRepresentations) throws IOException {
        prepareAdminToken();
        RetrofietUserService service
                = RetrofietServiceGenerator.createService(RetrofietUserService.class, adminToken);
        log.info("id truyen vao la: " + id);
        log.info("client truyen vao la: " + client);
        Response<Void> response
                = service.saveClientRole(id,client, clientRoleRepresentations).execute();
        if (!response.isSuccessful()) {
            if (response.code() == HttpStatus.UNAUTHORIZED.value()
                    || response.code() == HttpStatus.FORBIDDEN.value()) {
                adminToken = null;
